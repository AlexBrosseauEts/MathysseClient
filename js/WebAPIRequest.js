...
function webAPI_HEAD(successCallBack, errorCallBack) {
    $.ajax({
        ...
        error: function(jqXHR) {
            errorCallBack(jqXHR.status);
            console.log("webAPI_GET_ALL_EXPENSES_HEAD - error");
        }
    });
}
...
function webAPI_GET_ALL_EXPENSES(queryString, successCallBack, errorCallBack) {
    $.ajax({
        url: apiBaseURL + "/api/expenses" + queryString,
        type: 'GET',
        contentType:'text/plain',
        data:{},
        success: (data, status, xhr) => { 
            let ETag = xhr.getResponseHeader("ETag");
            successCallBack(data, ETag); 
            console.log("webAPI_GET_ALL_EXPENSES - success", data); 
            console.log(`ETag: ${ETag}`);
        },
        error: function(jqXHR) {
            errorCallBack(jqXHR.status);
...
function webAPI_GET_PRIVATE_MONEY(id, successCallBack, errorCallBack) {
    $.ajax({
        url: apiBaseURL + "/api/money?UserId=" + id,
        type: 'GET',
        contentType:'text/plain',
        data:{},
        success: (data, status, xhr) => { 
            let ETag = xhr.getResponseHeader("ETag");
            successCallBack(data, ETag); 
            console.log("webAPI_GET_PRIVATE_MONEY - success", data); 
            console.log(`ETag: ${ETag}`);
        },
        error: function(jqXHR) {
            errorCallBack(jqXHR.status);
            console.log("webAPI_GET_PRIVATE_MONEY - error");
        }
    });
}
...
function webAPI_POST_EXPENSES( data , successCallBack, errorCallBack) {
    $.ajax({
        url: apiBaseURL + "/api/expenses",
        type: 'POST',
        headers: getBearerAuthorizationToken(),
        ...
        error: function(jqXHR) {
            errorCallBack(jqXHR.status);
            console.log("webAPI_POST_EXPENSES - error");
        }
    });
}
function webAPI_POST_MONEY( data , successCallBack, errorCallBack) {
    
    console.log("Le data de money: "+ data.FullAmount);
    $.ajax({
        url: apiBaseURL + "/api/money",
        type: 'POST',
        headers: getBearerAuthorizationToken(),
        contentType:'application/json',
        data: JSON.stringify(data),
...
function webAPI_PUT_EXPENSES(data, successCallBack, errorCallBack) {
    $.ajax({
        url: apiBaseURL + "/api/expenses" + "/" + data.Id,
        type: 'PUT',
        headers: getBearerAuthorizationToken(),
        contentType:'application/json',
        data: JSON.stringify(data),
        ...
        error: function(jqXHR) {
            errorCallBack(jqXHR.status);
            console.log("webAPI_PUT_EXPENSES - error");
        }
    });
}
function webAPI_PUT_MONEY(data, successCallBack, errorCallBack) {
    console.log("Data Id: "+data.id);
    $.ajax({
        url: apiBaseURL + "/api/money" + "/" + data.Id,
        type: 'PUT',
        headers: getBearerAuthorizationToken(),
        contentType:'application/json',
        data: JSON.stringify(data),
